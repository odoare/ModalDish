...
$\sqrt{\zeta_{\mathrm{ref}}/\zeta_k}$ (clamped to $[0.5, 32]$,
$\zeta_{\mathrm{ref}}=10^{-2}$), the loudness compensation inherited from
the MechanOdd modal resonators. Retunes triggered by parameter changes
...
The band-pass resonators are output-normalised (constant peak gain), so
their internal states are \emph{not} calibrated modal displacements: a
naive $\sum_k g_k \langle y_k^2\rangle$ over filter outputs $y_k$
underestimates the stretching by orders of magnitude (impulse ring
amplitudes scale with $\zeta_k$). FemPlate therefore drives the effect from
the \emph{audible output} and expresses it as a dimensionless
\emph{relative stiffening of mode 1}:
%
\begin{equation}
  \gamma \;=\; \min\!\bigl(\gamma_{\max},\;
        \alpha\, \cdot \mathrm{nonlin} \cdot \langle w^2(\bm{x}_o)\rangle \bigr),
  \qquad
  T_{\mathrm{dyn}} = \gamma\, \frac{\omega_1^2(T_{\mathrm{knob}})}{g_1},
  \label{eq:gamma}
\end{equation}
%
with $\alpha = 8$, $\gamma_{\max}=4$, and $\langle\cdot\rangle$ a 20-ms
one-pole envelope. By construction $\omega_1^2 \mapsto
\omega_1^2\,(1+\gamma)$ exactly, and every other mode follows its own
physical sensitivity $g_k$: $\gamma$ is plate-independent ($\gamma = 0.4$
always means ``mode 1 up $\approx 3$ semitones''), which makes the knob
calibration robust across geometries. Losing the per-mode weighting is a
second approximation on top of Berger's; the qualitative behaviour ---
glide amplitude tracking the ring energy --- is preserved.
...
%
\begin{equation}
  w^{(b)}(t) = \sum_{c = b-W}^{b-1}\; \sum_{k \in \mathcal{B}_c}
              \varphi_k(\bm{x}_o)\, c_k\, y_k(t),
  \qquad
...
settings). The source \emph{window} matters: summing all lower bands
...
\paragraph{Material-damping compensation.}
The audible strength of the cascade depends steeply on the material
damping: large $\zeta_m$ widens the target resonances (overlap floor) and
raises their bandwidth compensation, so a knob value that is mild at
feather damping saturates the output when hit and pickup are close.
Listening tests produced qualitative ``maximum usable cascade'' limits
%
\begin{center}
\begin{tabular}{cccccc}
\toprule
$\zeta_m$ & $2.4\cdot10^{-3}$ & $10^{-3}$ & $10^{-4}$ & $10^{-5}$ & $7\cdot10^{-6}$\\
max cascade & 0.05 & 0.1 & 0.4 & 0.9 & 1\\
\bottomrule
\end{tabular}
\end{center}
%
which are nearly linear in log-log; the fitted power law
%
\begin{equation}
  s(\zeta_m) \;=\; \min\!\left(1,\;
     \bigl(2.3\cdot10^{-5}/\zeta_m\bigr)^{0.64}\right)
\end{equation}
%
reproduces the table within $\sim$10\%. The effective cascade amount is
$\mathrm{cascade} \times s(\zeta_m)$ (applied to the injection, the
overlap floor and the depletion alike), so the knob keeps its full travel
and its maximum tracks the acceptability limit at every damping setting.
