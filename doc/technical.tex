...
%
\begin{equation}
  w^{(b)}(t) = S \sum_{c = b-W}^{b-1} \frac{1}{n_c}
              \sum_{k \in \mathcal{B}_c}
              \varphi_k(\bm{x}_o)\, v_k\, y_k(t),
  \qquad
...
settings). The source is the plate's \emph{motion}: $v_k = y_k$'s modal
velocity scale $\zeta_{\mathrm{ref}}/(\zeta_k \nu_k)$, normalised per band
by $n_c = \sum_{k \in \mathcal{B}_c} 1/\nu_k$ and by one global gain
$S = 3$ (\S\ref{par:drive}). The source \emph{window} matters: summing all lower bands
...
real synthesis and asserts the invariance, so the artefact cannot return
unnoticed:
%
\begin{center}
\begin{tabular}{lcc}
