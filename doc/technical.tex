...
$300$--$3000$) and only the lowest $m \le 128$ modes wanted (capped at $\sim n/6$ so the top modes stay resolved by the mesh), the solver is
...
output by $\varphi_k(\bm{x}_o)$. Up to 128 modes are active; the bank is
...
128 modes. FemPlate implements two lightweight surrogates, one for each
phenomenon.
...
$W_k y_k^2$. Measured on the default plate, the peak driver varies by less
than $12\%$ over $\zeta_v, \zeta_m \in [10^{-5}, 10^{-3}]$, where the
earlier pickup-amplitude driver $\langle w^2(\bm{x}_o)\rangle$ varied by a
factor $100$ --- and by a factor $1300$ between a struck plate and the
same plate driven through its audio input. Driving the effect from a
global, calibrated quantity rather than from the amplitude at the output
point also makes the glide independent of where the pickup sits: on a node
it used to vanish, and moving the pickup moved the pitch.
...
summed for the output, $\varphi_k(\bm{x}_o)\,c_k\,y_k$. That is what
FemPlate used until it was measured, and it made the effect depend steeply
on \emph{damping}, in the wrong direction: turning a damping knob up made
the shimmer louder. The mechanism is \eqref{eq:compensation}'s
$c_k = \sqrt{\zeta_{\mathrm{ref}}/\zeta_k}$. A band-pass ring has peak
$2\zeta_k\omega_k$, so the audible peak of a struck mode is
%
\begin{equation}
...
\end{equation}
%
the compensation equalises each mode's ring \emph{energy} by trading
duration for level, so a more damped mode peaks \emph{higher}. That is
sound design, not physics --- a struck plate's modal amplitude is set by
the impulse, and damping can only remove energy --- and the ladder then
cubes the difference. Measured on the default plate (one strike, 4\,kHz and
up, at 300\,ms), the shimmer rose by $38.7$\,dB across the Viscous knob's
range while the plate was merely decaying faster; the low band's peak
followed $\sqrt{\zeta_k}$ to within $0.2$\,dB of prediction
($+18.7$\,dB measured, $+18.9$ predicted, for $\zeta$ increased $143\times$).

Weighting the source by $v_k \propto 1/(\zeta_k\nu_k)$ instead removes the
damping entirely, since $y_k \propto \zeta_k$: what is left is the modal
velocity, which is what the plate actually does. The per-band divisor
$n_c$ then restores the \emph{frequency} balance that the compensation had
been supplying by accident --- physical modal amplitudes fall steeply with
frequency, and a raw velocity sum outweighs band 1 with band 0 by a factor
$\sim\!900$, starving the upper rungs --- but with a constant fixed by the
mode indices alone, so no damping re-enters. Each rung is then driven by a
band-mean modal velocity.

\paragraph{Nor at the pickup.}
The same sum used to carry $\varphi_k(\bm{x}_o)$, so the entire cascade
followed the output-point parameters: a mode nodal at the pickup drove
nothing, however hard the plate was moving there, and dragging the output
point across the plate moved the drive by $18.2$\,dB. A real plate's cubic
coupling does not know where the microphone is. The weight is now the
constant $1/\sqrt{|\Omega|}$, the typical size of a mass-normalised mode
shape ($\int_\Omega \varphi_k^2 \dd A = 1$), which keeps the drive's level
the same on any geometry while making it depend on no listening position
at all --- measured across five pickup positions, the high-mode motion the
cascade creates is now identical to the last digit. What the pickup still
changes is what one \emph{hears} of that motion, which is what a pickup is
for. The \emph{injection} keeps its $\varphi_l(\bm{x}_h)$: the hit point is
where the plate deflects most, the one position the nonlinearity has a
physical claim to, and it is what ties the cascade's character to how the
plate is played.

With that source the drive is flat to a few dB across both damping knobs,
and the fitted power law $\min(1, (2.3\cdot10^{-5}/\zeta_m)^{0.64})$ that
used to scale the cascade amount --- an empirical correction for the
material half of the same artefact --- has been removed: the knob now acts
directly. \code{Tests/CascadeMeasure.cpp} renders these sweeps through the
real synthesis and asserts both invariances, so the artefacts cannot
return unnoticed:
%
\begin{center}
\begin{tabular}{lcc}
\toprule
Shimmer at 300\,ms (dB) & spread, audio-driven & spread, motion-driven \\
\midrule
Viscous $\zeta_v \in [10^{-5}, 10^{-3}]$  & $29.6$ & $3.6$ \\
Material $\zeta_m \in [7\cdot10^{-6}, 3\cdot10^{-4}]$ & $39.3$ & $2.0$ \\
\bottomrule
\end{tabular}
\end{center}
%
while the Cascade knob still spans $18$\,dB over its travel and a
force-$10$ hit still shimmers $45$\,dB more than a force-$1$ one: the
...
Two earlier designs closed a feedback loop through the full output. Around
mode $k$'s resonance such a loop transmits with gain
...
Even a 128-mode FEM bank remains a sparse comb at the top of the
...
(to 256 modes in all) with \emph{synthetic} modes above the FEM set,
...
The numerical core (\code{FemMesh}, \code{PlateModes}) is pure C++17 with
no JUCE dependency and is validated by a console test suite; it lives in
the \code{FxmeTools} submodule (\code{FxmeTools/acoustics}) for reuse in
other projects.
