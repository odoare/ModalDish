% ----------------------------------------------------------------------------
% ModalDish — technical documentation
%
...
\title{ModalDish: finite-element modal synthesis of plates and membranes
...
\date{July 2026 --- ModalDish v0.1}

\begin{document}
\maketitle

\begin{abstract}
ModalDish is an audio plugin that simulates the linear and weakly nonlinear
...
a non-conforming (Morley) finite-element discretisation and a subspace
eigensolver over sparse operators in a bandwidth-reducing ordering, both
implemented in the reusable, JUCE-free library
...
Poisson ratio ($\nu = 0.3$ in ModalDish), $c_1$ a viscous (air/support)
...
delicate. ModalDish instead uses the \emph{Morley triangle}
...
in \S\ref{sec:validation}. Crucially for ModalDish, its two DOF families are
...
\item Form $A = K + T_0 G$ and the shifted operator $P = A + \sigma M$ with
  ...
  modes in $A$'s null space. $P$ is Cholesky-factored once, in the profile
  storage described below rather than densely.
...
Figure~\ref{fig:signalpath} shows where everything in this section and
the next sits in the running plugin, from a note arriving to a sample
leaving.
...
\begin{tikzpicture}[
  ...
  blk/.style={draw=black!55, rounded corners=2pt, align=center,
              inner sep=3pt, text width=32mm, minimum height=8mm},
  wide/.style={blk, text width=46mm},
  mdl/.style={blk, fill=black!7},
  exc/.style={blk, fill=orange!14},
  tun/.style={blk, fill=blue!11},
  bnk/.style={wide, fill=blue!16},
  fbk/.style={blk, fill=red!9},
  outb/.style={wide, fill=green!13},
  term/.style={blk, fill=white, rounded corners=8pt, minimum height=6mm},
  ar/.style={->, thick, black!65},
  pub/.style={->, thick, dashed, black!50},
  lbl/.style={font=\scriptsize, black!55, inner sep=1.5pt}
]

% ---- left column: the model, its tuning, and the Berger driver --------
\node[mdl] (shape) at (-5.2,-1.0) {Shape\\outline $+$ boundary segments};
\node[mdl] (mesh)  at (-5.2,-2.7) {Delaunay mesh\\\emph{Grid}~(\S\ref{sec:mesh})};
\node[mdl] (solve) at (-5.2,-4.4) {Morley FEM eigensolve\\\emph{Modes}~(\S\ref{sec:eig})};
\node[mdl] (tail)  at (-5.2,-6.0) {Statistical tail~(\S\ref{sec:tail})};
\node[mdl] (model) at (-5.2,-7.4) {Modal model\\$\lambda_k,\ \bm\varphi_k,\ g_k$};
\node[tun] (omega) at (-5.2,-9.2) {$\omega_k$\\\emph{Frequency}, MIDI pitch,\\\emph{Glide}, \emph{Tension}~(\S\ref{sec:tension})};
\node[tun] (zeta)  at (-5.2,-10.9) {$\zeta_k$\\\emph{Viscous}, \emph{Material},\\\emph{Overlap} floor~(\S\ref{sec:damping})};
\node[fbk] (berger) at (-5.2,-12.7) {Berger driver\\$\gamma$ from plate stretching~(\S\ref{sec:berger})};

\draw[ar] (shape) -- (mesh);
\draw[ar] (mesh) -- (solve);
\draw[ar] (solve) -- (tail);
\draw[ar] (tail) -- (model);
\draw[ar] (berger.west) -- ++(-0.55,0) |- node[lbl,pos=0.25,rotate=90,above] {dynamic tension} (omega.west);

% ---- excitation -------------------------------------------------------
\node[term] (midi)  at (0,-1.0) {MIDI in: notes, CC};
\node[term] (audin) at (3.6,-1.0) {Audio in};
\node[exc] (route)  at (0,-2.7) {Note routing\\\emph{Src Chan} / \emph{Freq Chan}~(\S\ref{sec:notes})};
\node[exc] (ingain) at (3.6,-2.7) {\emph{In Gain}};
\node[exc] (map)    at (0,-4.6) {Source mapping\\$\min + a\,(\max-\min)$\\position, hammer, force};
\node[exc] (sends)  at (3.6,-4.6) {Per-source\\\emph{In Vol} / \emph{In Bal}};
\node[exc,wide] (drive) at (1.8,-6.6) {Modal drive: half-sine pulses and input, weighted by $\bm\varphi_k(\mathbf{x})$~(\S\ref{sec:excitation})};

\draw[ar] (midi) -- (route);
\draw[ar] (audin) -- (ingain);
\draw[ar] (route) -- (map);
\draw[ar] (ingain) -- (sends);
\draw[ar] (map.south) -- ++(0,-0.45) -| (drive.north);
\draw[ar] (sends.south) -- ++(0,-0.45) -| (drive.north);

% ---- the bank ---------------------------------------------------------
\node[bnk] (bank) at (1.8,-8.4) {Modal filter bank\\one constant-peak band-pass per mode~(\S\ref{sec:filterbank})};
\draw[ar] (drive) -- (bank);
\draw[pub] (model.east) -- ++(0.7,0) |- node[lbl,pos=0.72,above] {publish} (bank.north west);
\draw[ar] (omega.east) -- ++(0.5,0) |- (bank.west);
\draw[ar] (zeta.east)  -- ++(0.3,0) |- ([yshift=-2.5mm]bank.west);

% ---- feedback ---------------------------------------------------------
\node[fbk] (casc) at (7.6,-8.4) {Cascade ladder\\8 bands, $\tanh^3$ carrier,\\gate, \emph{Window}~(\S\ref{sec:cascade})};
\node[fbk] (depl) at (7.6,-10.2) {\emph{Deplete}\\loss on the pumping bands};

\draw[ar] (bank.east) -- (casc.west);
\draw[ar] (bank.south west) -- ++(-1.15,0) |- (berger.east);
\draw[ar] (casc) -- (depl);
\draw[ar] (casc.north) -- ++(0,0.30) -- (4.9,-7.45) -- (4.9,-6.6) -- (drive.east);
\draw[pub] (depl.west) -- ++(-0.55,0) |- node[lbl,pos=0.3,left,align=center] {state\\decay} ([yshift=-1mm]bank.east);

% ---- output -----------------------------------------------------------
\node[outb] (pmix)   at (1.8,-10.3) {Pickup mix: $\bm\varphi_k(\mathbf{y}_p)$, \emph{Level}, \emph{Pan}, tail spread~(\S\ref{sec:tailspread})};
\node[outb] (og)     at (1.8,-11.9) {\emph{Out Gain}};
\node[outb] (hpf)    at (1.8,-13.2) {20\,Hz Butterworth highpass};
\node[term] (stereo) at (1.8,-14.5) {Stereo out};
\node[mdl]  (field)  at (7.6,-11.9) {Plate view\\displacement / velocity};
\node[outb,blk] (meters) at (7.6,-13.2) {Peak meters};

\draw[ar] (bank) -- (pmix);
\draw[ar] (pmix) -- (og);
\draw[ar] (og) -- (hpf);
\draw[ar] (hpf) -- (stereo);
\draw[pub] ([yshift=2mm]bank.east) -- (4.4,-8.2) -- (4.4,-11.9) -- (field.west);
\draw[ar] (hpf.east) -- (meters.west);

\end{tikzpicture}
}
\caption{The complete signal path. Grey: the model, built on the message and
background threads once per plate. Orange: excitation. Blue: the filter bank
and what tunes it. Red: the two nonlinear feedback paths. Green: output.
Dashed arrows carry state rather than signal. Everything but the grey column
runs per sample on the audio thread. Italics name the control as it is
...
ModalDish realises each oscillator as a second-order IIR resonator, exactly
...
Muting the modes is not by itself enough to keep the sub-audio band clear,
because $f_1$ reaches down to $1$\,Hz and a live mode just above the floor
is a band-pass whose lower skirt falls at only $6$\,dB/oct, while the
half-sine hammer pulse carries DC. The output therefore passes a fixed
second-order Butterworth highpass at $20$\,Hz, the last stage before the
bus. Its double zero at the origin removes an offset absolutely rather than
attenuating it, it adds $12$\,dB/oct below the corner ($-12.3$\,dB at
$10$\,Hz, $-24.1$\,dB at $5$\,Hz), and it is within $0.44$\,dB of flat from
$35$\,Hz up, so it does not touch what the plate is meant to produce. The
meters read the buffer after it, so what they show is what leaves.
...
A source does not carry fixed excitation parameters but three
\emph{mappings}, one each for the strike position, the contact time and the
force. Each is a pair of endpoints and a controller, and a strike evaluates
%
\begin{equation}
  u \;=\; u_{\min} + a\,(u_{\max} - u_{\min}),
  \qquad a \in [0,1],
  \label{eq:sourcemap}
\end{equation}
%
where $a$ is what the selected controller currently reads: zero when it is
off, the note's velocity shaped by the source's curve
($a = v^2$, $v$ or $\sqrt{v}$), or the latest value of a MIDI continuous
controller. No ordering is imposed on the pair, so $u_{\min} > u_{\max}$
simply inverts the mapping.

The position mapping is the interesting one physically. With
$\bm{x}_h = \bm{x}_a + a\,(\bm{x}_A - \bm{x}_a)$ the excitation
\emph{moves}, so playing harder changes which modes are driven rather than
only how strongly: the modal amplitudes go as $\varphi_k(\bm{x}_h)$, and a
plate struck near the rim and near the centre has two genuinely different
spectra. A fixed excitation point cannot produce both, and scaling a single
point's amplitude --- what a velocity-sensitive sampler does --- reproduces
neither. Equal endpoints, the default, recover the fixed point exactly. The
positional spread of \S\ref{sec:excitation} scatters about
$\bm{x}_h$, and the audio input is injected at $\bm{x}_a$ alone, a
continuous signal having no velocity to place it by.

Force ships as $u_{\min} = 0$, $u_{\max} = 1$ on velocity, which is
identically the $v \times \mathrm{Force}$ amplitude the hammer had before
\eqref{eq:sourcemap} existed.
...
a few hundred modes. ModalDish implements two lightweight surrogates, one for each
...
ModalDish reproduces this with a \emph{windowed} band ladder. The active
...
cascade the low modes \emph{lose} the energy they hand upward. ModalDish
...
receiving continuum up to Nyquist. ModalDish therefore fills the bank
...
holds. ModalDish uses it as the band's stereo placement,
%
\begin{equation}
...
The closest published work to ModalDish's objective is the real-time gong
...
\item \emph{Cost.} Reported timings are $0.15$--$0.57$\,s of computation per
  ...
  one, for grids of roughly $300$ to $720$ interior points. ModalDish's full
  bank --- 256
...
modal picture --- and would keep ModalDish's arbitrary geometry, since the
...
Figure~\ref{fig:signalpath} collects the whole chain in one place;
the table below says which thread each stage runs on.
...
State save / restore \S\ref{sec:persistence} & message & parameters, geometry and the modal cache \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Persistence: what a saved plate carries}
\label{sec:persistence}

A saved state --- a preset file or a host session, which are written by the
same route and therefore carry the same things --- holds three items:

\begin{enumerate}
\item the parameters;
\item the \emph{geometry}: the outline and the arc-parameterised boundary
  segments of \S\ref{sec:bc}, a few kilobytes;
\item the \emph{modal data}: $\lambda_k$, the tension sensitivities $g_k$ of
  \S\ref{sec:tension}, the mass-normalised FEM shapes $\bm\varphi_k$ and the
  parameters of the statistical tail of \S\ref{sec:tail}.
\end{enumerate}

The mesh is deliberately \emph{not} stored. Mesh generation
(\S\ref{sec:mesh}) is a deterministic function of the outline and the target
element size, both of which are stored, so rebuilding it on load costs less
than reading it would. Only its vertex count travels, as a check that the
stored shapes are indexed against the mesh that comes back.

The modal data is stored, and it is the only part large enough for the
decision to be interesting. Its size is one float per mesh vertex per FEM
mode,
%
\begin{equation}
  4\, m\, N_v \ \text{bytes},
  \qquad m = \min\!\left(256,\ \max\!\left(8,\ \tfrac{n}{6}\right)\right),
  \label{eq:cachesize}
\end{equation}
%
everything else (the $2 \times 1024$ scalars of $\lambda$ and $g$, four floats
per tail mode) being under $30$\,kB at any setting. Measured on the meshes the
Grid control actually produces:

\begin{center}
\begin{tabular}{rrrrr}
\toprule
& \multicolumn{2}{c}{shipped plate} & \multicolumn{2}{c}{largest plate in the canvas} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}
Grid & $N_v$ / $m$ & cache & $N_v$ / $m$ & cache \\
\midrule
$16$ & $149$ / $92$   & $0.10$\,MB & $227$ / $142$  & $0.18$\,MB \\
$24$ & $326$ / $207$  & $0.33$\,MB & $497$ / $256$  & $0.58$\,MB \\
$32$ & $569$ / $256$  & $0.66$\,MB & $858$ / $256$  & $0.98$\,MB \\
$48$ & $1258$ / $256$ & $1.42$\,MB & $1915$ / $256$ & $2.15$\,MB \\
\bottomrule
\end{tabular}
\end{center}

The blocks are written as base64 inside the state's XML and the whole is then
gzipped, and the two very nearly cancel, so the figures above are also what
reaches the disk.

Those megabytes buy the elimination of an eigensolve on every load. The times
of \S\ref{sec:eig} are the cost avoided: seconds at a moderate mesh, tens of
seconds at the top of the Grid range. That wait would be silence rather than
merely a delay, because the model publication of \S\ref{sec:filterbank} keeps
the \emph{previous} plate sounding while a new one is computed, and on a fresh
load there is no previous plate. The solve remains as the fallback, taken when
the cache is absent or when its vertex count disagrees with the rebuilt mesh.
